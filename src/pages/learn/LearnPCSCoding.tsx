...
export default function LearnPCSCoding() {
  const currentIndex = CHAPTERS.findIndex(c => c.path === ROUTES.LEARN.PCS_CODING)
  const prevChapter = currentIndex > 0 ? { path: CHAPTERS[currentIndex - 1].path, title: CHAPTERS[currentIndex - 1].title, icon: <Target className="w-4 h-4" /> } : undefined
  const nextChapter = currentIndex < TOTAL_CHAPTERS - 1 ? { path: CHAPTERS[currentIndex + 1].path, title: CHAPTERS[currentIndex + 1].title, icon: <Target className="w-4 h-4" /> } : undefined
  return (
    <LearnLayout
      title="概率星座整形与编码"
